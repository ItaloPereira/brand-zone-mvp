...
import { createPaletteSchema } from "./schema";

export const createPalette = async (formData: unknown) => {
  const validatedData = createPaletteSchema.parse(formData);
  ...
  return await db.$transaction(async (tx) => {
    ...
    const palette = await tx.colorPalettes.create({
      data: {
